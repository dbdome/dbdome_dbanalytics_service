...
-- Generates CategoryDefinitions.json structure from rootcause schema
-- Hierarchy: domains → areas → categories (issues)
-- IDs: domain=code, area=domain||area, category=domain||area||issue_id
...
category_data AS (
    SELECT DISTINCT
        i.domain_code || i.area_code || i.issue_id AS id,
        i.name,
        CASE
        ...
        (SELECT array_agg(f ORDER BY f)
         FROM (
             SELECT i.issue_id || '.json' AS f
             UNION ALL
             SELECT rc.root_cause_id || '.json'
...
SELECT json_build_object(
    ...
    'domains', (SELECT json_agg(row_to_json(d)) FROM domain_data d),
    'areas', (SELECT json_agg(row_to_json(a)) FROM area_data a),
    'categories', (SELECT json_agg(row_to_json(c)) FROM category_data c),
    'servers', (SELECT json_agg(DISTINCT s.server) FROM metrics.servers s WHERE s.is_active = true)
) AS result;
