...
def collect_all_metrics_mysql_queries(mysql_server, mysql_database, mysql_username, mysql_password, mysql_port, domain_filter=None, risk_filter=None, server_id=None):
    ...
    mysql_password = decrypt_secret(mysql_password)
  ...
  (
      SELECT  row_number() over (partition by rc.root_cause_id, rc.step_name
                                 order by case when lower(cm.db_vendor)   = 'mysql' then 0 else 1 end,
                                          case when lower(rc.vendor_name) = 'mysql' then 0 else 1 end) seq,
	...
	(
	  ...
	  where lower(db_vendor) in ('mysql')
        ...
        for _, query_row in queries_df.iterrows():  # Renamed to avoid confusion
            ...
            if is_self_activity(metric_name, query_row.get('query'), 'mysql'):
                db_write_log(f"Metric '{metric_name}' suppressed as DBDOME self-activity/benign inventory - not stored, not alerted.",
                             0, "collect_all_metrics_mysql_queries", mysql_server, port=mysql_port)
                continue
            ...
            try:
                ...
                if comparison_data.get("matched") is True:
                    ...
                    try:
                        ...
                        db_write_log(f"SIEM alert failed for '{metric_name}': {siem_ex}", 0, "collect_all_metrics_mysql_queries", mysql_server, port=mysql_port)
