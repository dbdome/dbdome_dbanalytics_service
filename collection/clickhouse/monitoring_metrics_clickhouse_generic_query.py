"""ClickHouse generic-metric collector.

Mirrors the other vendor generic collectors (mysql/postgres/…): pulls the active
clickhouse detection queries from the catalog, runs each against the target
ClickHouse instance, stores the result in monitoring.general_metric_metadata_
results and drives the alert / SIEM / diagnosis paths — all Postgres-side logic
is identical to the mysql collector and copied verbatim.

The ONE vendor-specific difference is transport: ClickHouse has no bundled
SQLAlchemy dialect and no driver in the build venv, so this collector talks to
the ClickHouse HTTP interface (default port 8123) with `requests` — POST the SQL
with `FORMAT JSON` and shred the JSON `data`/`meta` into a DataFrame. No new pip
dependency (requests is already bundled).
"""
import pandas as pd
...
# ---------------------------------------------------------------------------
# ClickHouse HTTP transport (no SQLAlchemy dialect / driver needed)
# ---------------------------------------------------------------------------
def _ch_base_url(server, port):
    host = str(server).split(':')[0]
    return f"http://{host}:{int(port or 8123)}/"


def _ch_query(base_url, user, password, database, sql, timeout):
    """Run one SQL statement over the ClickHouse HTTP interface, return a DataFrame.

    Appends `FORMAT JSON` when the statement has no explicit FORMAT, then shreds
    the JSON `meta`/`data` into a DataFrame preserving column order. Auth via the
    X-ClickHouse-User/Key headers; database via the `database` query param.
    """
    q = str(sql).strip().rstrip(';').strip()
    if not re.search(r'\bformat\s+\w+\s*$', q, re.IGNORECASE):
        q = q + "\nFORMAT JSON"
    ...
    resp = requests.post(base_url, params=params, data=q.encode('utf-8'),
                         headers=headers, timeout=timeout)
    resp.raise_for_status()
    payload = resp.json()
    cols = [c.get('name') for c in payload.get('meta', [])]
...
def collect_all_metrics_clickhouse_queries(ch_server, ch_database, ch_username, ch_password, ch_port, domain_filter=None, risk_filter=None, server_id=None):
    """Retrieve the active clickhouse metric queries from the catalog, run each
    against the target ClickHouse instance (HTTP), and store results in
    monitoring.general_metric_metadata_results."""

    # Target-DB password is stored encrypted; decrypt_secret is idempotent.
    ch_password = decrypt_secret(ch_password)

    base_url = _ch_base_url(ch_server, ch_port)
    ch_server_host = str(ch_server).split(':')[0]
  ...
  (
      SELECT  row_number() over (partition by rc.root_cause_id, rc.step_name
                                 order by case when lower(cm.db_vendor)   = 'clickhouse' then 0 else 1 end,
                                          case when lower(rc.vendor_name) = 'clickhouse' then 0 else 1 end) seq,
      ...
      WHERE cm.is_active = true
        AND lower(cm.db_vendor) = 'clickhouse'
        and rc.vendor_name in ('clickhouse')
  ...
  WHERE seq = 1
	...
	(
	  ...
	  where lower(db_vendor) in ('clickhouse')
	    and is_active = true
        ...
        if queries_df.empty:
            db_write_log("No active clickhouse metric queries found to process.", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
            return 1
        ...
        try:
            ...
            apply_parameter_overrides(pg_engine, f"{ch_server_host}", queries_df)
        except Exception as _ovr_ex:
            db_write_log(f"threshold override overlay skipped: {_ovr_ex}", 0,
                         "threshold_overrides", f"{ch_server_host}")
        ...
        for _, query_row in queries_df.iterrows():
            ...
            if is_self_activity(metric_name, query_row.get('query'), 'clickhouse'):
                db_write_log(f"Metric '{metric_name}' suppressed as DBDOME self-activity/benign inventory - not stored, not alerted.",
                             0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                continue

            print(f"Running metric: {metric_name}")
            db_write_log(f"Metric '{metric_name}'  Running metric", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
            try:
                ...
                df = _ch_query(base_url, ch_username, ch_password, ch_database, metric_query, _qt)
                ...
                if insert_payloads:
                    _bulk_insert(insert_payloads, pg_engine, ch_server_host, ch_port)
                    db_write_log("All metrics inserted successfully (bulk).", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                ...
                if comparison_data.get("matched") is True:
                    ...
                    try:
                        ...
                        db_write_log(f"alert_log insert failed for '{metric_name}': {alog_ex}", 0,
                                     "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                    ...
                    try:
                        ...
                        if not alert_authorised:
                            db_write_log(f"Alert skipped for '{metric_name}' - not authorised (domain={domain_filter}, risk={_risk_level})", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                        else:
                            try:
                                ...
                                db_write_log(f"Alert sent for '{metric_name}' (matched, authorised)", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                            except Exception as alert_ex:
                                db_write_log(f"Alert failed for '{metric_name}': {alert_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                    except Exception as auth_ex:
                        db_write_log(f"Alert auth check failed for '{metric_name}': {auth_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                    ...
                    try:
                        ...
                        db_write_log(f"SIEM alert failed for '{metric_name}': {siem_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                    ...
                    try:
                        with pg_engine.connect() as pg_check:
                            ...
                            if diag_row is not None:
                                manage_diagnosys_alerts(query_row.get('root_cause_id') or metric_name, metric_query, comparison, ch_server_host, metric_metadata_json, _server_id=server_id)
                    except Exception as diag_ex:
                        db_write_log(f"Diagnosis evidence check failed for '{metric_name}': {diag_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
            except Exception as metric_ex:
                db_write_log(f"Metric '{metric_name}' failed with error: {metric_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
                manage_diagnosys_alerts(query_row.get('root_cause_id') or metric_name, metric_query, None, ch_server_host, [], status="failed", _server_id=server_id)
                continue

        return 1

    except Exception as e:
        db_write_log(f"collect_all_metrics_clickhouse_queries failed with error: {e}", 0, "collect_all_metrics_clickhouse_queries", ch_server_host, port=ch_port)
        return 0


def _bulk_insert(insert_payloads, pg_engine, ch_server, ch_port):
    ...
    try:
        ...
        db_write_log(f"Bulk insert done for {len(insert_payloads)} metrics.", 0, "collect_all_metrics_clickhouse_queries", ch_server, port=ch_port)

    except Exception as bulk_ex:
        db_write_log(f"Bulk insert failed: {bulk_ex}", 0, "collect_all_metrics_clickhouse_queries", ch_server, port=ch_port)
