...
def collect_metric_mssql_active_sessions(mssql_server,mssql_servername  , mssql_database , mssql_username , mssql_password , mssql_driver,auth_type , mssql_port=None):
    server = mssql_server
...
"""
    ...
    try:
            ...
            with postgres_engine.begin() as conn:                                                                  
                conn.execute(
                text("INSERT INTO monitoring.active_sessions (\
                      server,session_id,blocking_session_id,wait_time,wait_type,last_wait_type,wait_resource,transaction_isolation_level,lock_timeout )\
                      VALUES (:server,:session_id,:blocking_session_id,:wait_time,:wait_type,:last_wait_type,:wait_resource,:transaction_isolation_level,:lock_timeout)"),
                        [{
                      "server":row[0],
                      "session_id":row[1],
                      "blocking_session_id":row[2],
                      "wait_time":row[3],
                      "wait_type":row[4],
                      "last_wait_type":row[5],
                      "wait_resource":row[6],
                      "transaction_isolation_level":row[7],
                      "lock_timeout":row[8]
                       } for row in df.itertuples(index=False)]
)
    except Exception as e:                 
                    db_write_log(f"collect_metric_mssql_active_sessions failed with error:{e}"   ,0,"collect_metric_mssql_active_sessions" , servername, port=mssql_port)
    finally:
            db_write_log(f"✅ collect_metric_mssql_active_sessions Data sync complete."   ,0,"collect_metric_mssql_active_sessions" , servername, port=mssql_port)
            raw_conn.close()
    return  1
    return 0;
