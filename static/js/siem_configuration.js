document.addEventListener('DOMContentLoaded', function() {
    ...
    // Confetti colors: glowing teal, glowing purple, glowing pink, and glowing white
    const colors = ['#00FFE5', '#9D4EDD', '#FF69B4', '#FFFFFF']; // glowing teal, glowing purple, glowing pink, glowing white
    
    // Create confetti particle with enhanced properties for upward movement
    function createParticle(x, y) {
        return {
            x: x,
            y: y,
            vx: (Math.random() - 0.5) * 4, // Horizontal spread
            vy: -Math.random() * 4 - 3, // Upward velocity (negative Y)
            ...
            floatSpeed: -0.5 - Math.random() * 0.5 // Upward float
        };
    }
    ...
    function animateConfetti() {
        ...
        for (let i = particles.length - 1; i >= 0; i--) {
            ...
            // Update position with upward movement (rising from form)
            particle.x += particle.vx * 0.8; // Slow down horizontal movement
            particle.y += particle.vy;
            particle.vy += 0.05; // Light gravity
            particle.vy += particle.floatSpeed * 0.1; // Upward float effect
            ...
            // Very enhanced glow effect for glowing colors
            ctx.shadowBlur = 25;
            ...
            // Remove dead particles (rising upward, check top of screen)
    ...
    // Trigger enhanced confetti burst rising upward from form
    function burstConfetti(buttonX, buttonY) {
        ...
        // Create multiple bursts rising upward from button position
        for (let i = 0; i < particleCount; i++) {
            // Start from button position with some randomness
            const offsetX = (Math.random() - 0.5) * 40;
            const offsetY = (Math.random() - 0.5) * 40;
            const startX = buttonX + offsetX;
            const startY = buttonY + offsetY;
            
            // Create particle rising upward from form
