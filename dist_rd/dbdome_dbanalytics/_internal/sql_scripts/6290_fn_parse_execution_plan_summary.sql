...
-- Renamed to *_xml: this is the raw-plan-XML parser. monitoring.parse_execution_plan_summary(text)
-- is now the BY-QUERY entry point (see 6390) and calls this internally.
DROP FUNCTION IF EXISTS monitoring.parse_execution_plan_summary_xml(text);

CREATE OR REPLACE FUNCTION monitoring.parse_execution_plan_summary_xml(p_plan text)
RETURNS TABLE (
...
ALTER FUNCTION monitoring.parse_execution_plan_summary_xml(text) OWNER TO dbdome_adm;
