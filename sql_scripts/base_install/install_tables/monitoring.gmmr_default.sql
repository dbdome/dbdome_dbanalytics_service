-- Idempotent install for monitoring.gmmr_default
-- Auto-generated from dbanalytics_install.backup. Safe to re-run (psql -f).

CREATE SCHEMA IF NOT EXISTS monitoring;

CREATE TABLE IF NOT EXISTS monitoring.gmmr_default (
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint con
        ...
        WHERE n.nspname = 'monitoring' AND c.relname = 'gmmr_default'
          AND con.conname = 'gmmr_default_pkey') THEN
        ALTER TABLE ONLY monitoring.gmmr_default
    ADD CONSTRAINT gmmr_default_pkey PRIMARY KEY (entry_date, id);
    END IF;
END $do$;

CREATE INDEX IF NOT EXISTS gmmr_default_entry_date_idx ON monitoring.gmmr_default USING brin (entry_date);
CREATE INDEX IF NOT EXISTS gmmr_default_metric_name_entry_date_idx ON monitoring.gmmr_default USING btree (metric_name, entry_date DESC);
CREATE INDEX IF NOT EXISTS gmmr_default_server_metric_name_entry_date_idx ON monitoring.gmmr_default USING btree (server, metric_name, entry_date DESC);
...
BEGIN
    IF to_regclass('monitoring.general_metric_metadata_results') IS NOT NULL
       AND to_regclass('monitoring.gmmr_default') IS NOT NULL
