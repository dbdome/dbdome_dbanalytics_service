-- =============================================================================
-- 5047_acc_011_rc02_whoisactive_single_select.sql
-- SEC-SQL-ACC-011-RC02 (sqlserver): swap the detection to the single-SELECT
-- "Who Is Active" snapshot.
--
-- One statement (no temp tables, no GO, no '--' comments -- the collector runs
-- it raw and metrics.v_custom_metrics flattens \n -> space). Returns running
-- requests, sleeping sessions holding an open transaction, sessions whose last
-- request finished in the last 60s, and a UNION ALL plan-cache branch for
-- statements that ran in the last 60s after their session disconnected.
-- Excludes this session (@@SPID) and any %dbdome% login. The companion view
-- monitoring.v_sec_sql_acc_011_rc02 (1055) maps these output keys.
--
-- Idempotent; matched by (vendor_slug + name LIKE '%ACC-011-RC02%').
-- =============================================================================
